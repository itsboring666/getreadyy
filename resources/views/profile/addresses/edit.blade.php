...
@section('title', 'Edit Address | GET READY')
...
<div class="gr-wardrobe" style="padding: 60px 24px 100px;">
    <div style="max-width: 800px; margin: 0 auto;">
        ...
        <div style="margin-bottom: 36px; border-bottom: 1px solid var(--border); padding-bottom: 20px;">
            <div style="font-family: var(--font); font-size: 10px; font-weight: 700; color: var(--primary); letter-spacing: 0.12em; text-transform: uppercase; margin-bottom: 8px;">ADDRESS BOOK // MODIFY ENTRY</div>
            <h1 style="font-family: var(--font-heading); font-size: 36px; font-weight: 700; color: var(--text); margin: 0; letter-spacing: 0.04em;">EDIT COORDINATES</h1>
            <p style="color: var(--text-secondary); font-family: var(--font); font-size: 13px; text-transform: uppercase; letter-spacing: 0.06em; margin-top: 8px;">Modify existing delivery coordinates.</p>
        </div>

        <div style="background: #161616; border: 1px solid var(--border); box-shadow: 5px 5px 0px rgba(153,27,27,0.3); padding: 40px;">
            <form method="POST" action="{{ route('addresses.update', $address->id) }}" class="o-form">
                @csrf
                @method('PUT')
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 24px; margin-bottom: 32px;">
                    ...
                    <div style="grid-column: 1 / -1;">
                        ...
                        <input type="text" id="address_name" name="address_name" value="{{ old('address_name', $address->address_name) }}" required placeholder="e.g. Home, Work, Office"
                    ...
                    <div style="grid-column: 1 / -1;">
                        ...
                        <textarea id="address" name="address" rows="3" required
                                  ...
                                  onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='var(--border)'">{{ old('address', $address->address) }}</textarea>
                    ...
                    <div>
                        ...
                        <input type="text" id="zip" name="zip" value="{{ old('zip', $address->zip) }}" required
                               style="width: 100%; padding: 12px 16px; border: 1px solid var(--border); background: #1a1a1a; color: var(--text); font-family: var(--font); font-size: 13px; outline: none; transition: border-color 0.2s;"
                               onfocus="this.style.borderColor='var(--primary)'" onblur="this.style.borderColor='var(--border)'">
                    ...
                    <div style="grid-column: 1 / -1; display: flex; align-items: center; gap: 10px; padding-top: 8px; border-top: 1px dashed var(--border); margin-top: 8px;">
                        @if($address->is_default)
                            ...
                            <span style="font-family: var(--font); font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.08em; color: var(--primary);">This is your current default address.</span>
                        @else
                ...
                <div style="display: flex; gap: 16px; align-items: center; justify-content: flex-end; padding-top: 28px; border-top: 1px solid var(--border);">
                    ...
                    <button type="submit" class="gr-hero-btn-primary" style="padding: 14px 32px; font-size: 11px;">UPDATE COORDINATES →</button>
                </div>
            </form>
        </div>

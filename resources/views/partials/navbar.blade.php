    ...
    <div class="gr-header-inner">
        ...
        <nav class="gr-nav" aria-label="Main Navigation">
            <a href="{{ route('products.all') }}"
                class="gr-nav-link {{ request()->routeIs('products.all') ? 'active' : '' }}">Shop</a>
        ...
        <div class="gr-nav-icons" style="display: flex; align-items: center; gap: 16px;">
            ...
            {{-- Global Search Bar --}}
            <form action="{{ route('products.all') }}" method="GET" id="nav-search-form" class="gr-nav-search"
                ...
                <input type="text" id="nav-search-input" name="search" placeholder="Search products..."
                    ...
                    style="background: transparent; border: none; outline: none; font-family: var(--font); font-size: 12px; width: 150px; color: var(--text);">
                ...
                <div id="autocomplete-results"
                    style="display: none; position: absolute; top: calc(100% + 8px); right: -2px; background: #161616; border: 1px solid var(--border); box-shadow: 4px 4px 12px rgba(0,0,0,0.6); z-index: 1000; max-height: 400px; overflow-y: auto; width: 300px;">
            ...
            @auth
                ...
                @if(auth()->user()->user_type === 'admin')
                    <a href="{{ route('admin.dashboard') }}" class="gr-nav-icon" aria-label="Admin Dashboard"
                        title="Admin Panel">
            ...
            @auth
                <a href="{{ route('wishlist.index') }}" class="gr-nav-icon" aria-label="Wishlist" title="Wishlist"
                    style="position: relative;">
                    <i class="far fa-heart" aria-hidden="true"></i>
                    @if(auth()->user()->wishlistItems->count() > 0)
                        <span class="gr-cart-badge"
                            style="background: var(--accent); right: -6px; top: -6px;">{{ auth()->user()->wishlistItems->count() }}</span>
                    @endif
                </a>
            @endauth
            ...
            <a href="{{ route('cart') }}" class="gr-nav-icon" aria-label="Shopping Cart" title="Cart">
                ...
                @if(session()->has('cart') || auth()->check() && auth()->user()->cartItems()->count() > 0)
                    <span
                        class="gr-cart-badge">{{ auth()->check() ? auth()->user()->cartItems()->count() : count(session('cart', [])) }}</span>
                @else
            ...
            {{-- Mobile Toggle --}}
            <button class="gr-mobile-toggle" id="grMobileToggle" aria-expanded="false"
...
<div id="grDrawer" style="display:none; position:fixed; inset:0; z-index:1000;">
    ...
    <div
        style="position:absolute; top:0; right:0; bottom:0; width:300px; background:#161616; display:flex; flex-direction:column; border-left: 1px solid var(--border);">
        <div
            style="display:flex; justify-content:space-between; align-items:center; padding:16px 20px; border-bottom:1px solid var(--border-light);">
            <div
        ...
        <div style="padding:20px; border-top:1px solid var(--border-light);">
            @guest
                <a href="{{ route('login') }}" class="o-btn o-btn-outline"
                    style="width:100%; justify-content:center; margin-bottom:10px;">Sign In</a>
                <a href="{{ route('register') }}" class="o-btn o-btn-primary"
                    style="width:100%; justify-content:center;">Create Account</a>
            @endguest
            @auth
                <a href="{{ route('profile.show') }}" class="o-btn o-btn-ghost"
                    style="width:100%; justify-content:flex-start; margin-bottom:10px;">
                    <i class="far fa-user" aria-hidden="true"></i> My Account
                </a>
                <a href="{{ route('wishlist.index') }}" class="o-btn o-btn-ghost"
                    style="width:100%; justify-content:flex-start; margin-bottom:10px;">
                    <i class="far fa-heart" aria-hidden="true"></i> My Wishlist
                </a>
                <form method="POST" action="{{ route('logout') }}" style="margin:0;">
                    @csrf
                    <button type="submit" class="o-btn o-btn-ghost"
                        style="width:100%; justify-content:flex-start; color:var(--danger);">
